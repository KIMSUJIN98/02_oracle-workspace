...
TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), 'YY"년" MM"월" DD"일"') AS "생년월일",
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS "주빈번호 앞자리", SUBSTR(EMP_NO, 8, 7) AS "주빈번호 뒷자리",
TO_NUMBER(SUBSTR(EMP_NO, 1, 6)) + TO_NUMBER(SUBSTR(EMP_NO, 8, 7)) AS "주민번호의 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';
