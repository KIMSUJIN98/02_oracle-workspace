...
/*
    ...
    * 트랜젝션(TRANSACTION)
    ...
    - 데이터의 변경사항(DML 추가, 수정, 삭제)들을 하나의 트랜젝션에 묶어서 처리
      ...
      ROLLBACK; 진행 : 한 트랜젝션에 담겨있는 변경사항들을 삭제(취소) 한 후 마지막 COMMIT시점으로 돌아감
...
ROLLBACK;
-- 변경사항 취소되고, 트랜잭션도 없어짐. 데이터 다시 되살아난다.
...
-- 217, 216, 214 사원 지움
DELETE FROM EMP_01
WHERE EMP_ID IN (217, 216, 214);                                                 -- 만약 EMP_ID가 VARCHAR2 자료형이라면 숫자 >> VARCHAR2로 자동형변환된다. (현재 EMP_ID는 NUMBER형태)
...
-- DDL문을 수행하는 순간 트랜젝션이 실제 DB에 반영되고, (커밋) DDL문이 수행됨.               -- DDL문 수행 전, 자동적으로 커밋이 이루어짐 => 오토커밋이라 함.
