...
--------------------------------------------------------------------------------
/*
    3. UPDATE                                                                    // CRUD / C : CREATE => INSERT / R : READ => SELECT / U : UPDATE / D : DELETE
...
SELECT * FROM EMP_SALARY                                                         -- 데이터 양이 많을때 일단 변경하고자하는 데이터값만 찾아내서 확인.
WHERE EMP_NAME = '노옹철';

UPDATE EMP_SALARY
SET SALARY = 1000000 -- 3700000                                                  -- 백업하는 습관 반드시!!
WHERE EMP_NAME = '노옹철';
...
UPDATE EMP_SALARY
SET SALARY = 7000000, BONUS = 0.2 -- 8000000 0.3
WHERE EMP_NAME = '선동일';
...
UPDATE EMP_SALARY
SET BONUS = 0.3
WHERE EMP_NAME IN (SELECT EMP_NAME                                               -- =은 값이 하나인 경우에만! 값이 여러개인 경우 IN으로 한다. EMP_NAME값 비교시 서브쿼리 SELECT문은 EMP_NAME만!
                   FROM EMP_SALARY
                   JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
                   JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
                   WHERE LOCAL_NAME LIKE 'ASIA%');
...
UPDATE EMPLOYEE
SET JOB_CODE = 'J9'                                                              -- J9는 JOB_CODE에 없다.
WHERE EMP_NAME = '노옹철';
-- ORA-02291: integrity constraint (KH.SYS_C007163) violated - parent key not found
-- FOREIGN KEY 제약조건 위배!!
...
/*
    4. DELETE
       ...
       [표현식]
       DELETE FROM 테이블명
       [WHERE 조건]; --> WHERE절 제시 안하면 전체 행이 다 삭제됨
*/
-- 차은우 사원의 데이터 지우기
SELECT * FROM EMPLOYEE
WHERE EMP_NAME = '주지훈';                                                        -- (1) '차은우' 조회 후 삭제 / (2) '주지훈' 조회 후 삭제

--ROLLBACK;                                                                      -- COMMIT을 치면 롤백 불가. DELETE FROM EMPLOYEE; 실행 후 복구를 위한 롤백이었음(수행할 필요X)
...
WHERE DEPT_ID = 'D1';
-- ORA-02292: integrity constraint (KH.SYS_C007162) violated - child record found
-- 외래키 제약 조건 위반!!
-- D1의 값을 가져다 쓰는 자식 데이터가 있기 때문에 삭제가 안됨!!

SELECT * FROM
EMPLOYEE
WHERE DEPT_CODE = 'D3';                                                          -- (1) D1을 조회해보니 자식이 있음 => 삭제 불가 / (2) D3을 조회해보니 자식이 없음 => 삭제 가능

DELETE FROM DEPARTMENT                                                           -- 자식이 없으므로 오류없이 삭제됨
WHERE DEPT_ID = 'D3';

ROLLBACK;                                                                        -- 원본 훼손 방지 롤백
