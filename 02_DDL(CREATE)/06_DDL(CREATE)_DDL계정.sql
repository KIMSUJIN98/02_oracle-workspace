...
--------------------------------------------------------------------------------
/*
    2. 컬럼에 주석 달기 (컬럼에 대한 설명 같은거)                                        -- 테이블 생성시에 주석을 다는건 안될까? ㄴㄴ 생성 후에 따로 달아줘야함.
...
INSERT INTO MEM_UNIQUE VALUES(1, 'user01', 'pass01', '손흥민', null, null, null);
INSERT INTO MEM_UNIQUE VALUES(2, 'user01', 'pass02', '황희찬', null, null, null);
-- ORA-00001: unique constraint (DDL.MEMID_UQ) violated                          -- violated : 위배되다
...
INSERT INTO MEM_PRI2
VALUES(1, 'user02', 'pass02', '황희찬', null, null, null);                         -- MEM_NO, MEM_ID 둘 중에 하나라도 다르면 가능함. (인스타그램 좋아요, 구독 등)

INSERT INTO MEM_PRI2
VALUES(1, 'user01', 'pass01', '이강인', null, null, null);                         -- MEM_NO, MEM_ID 모두 일치하므로 오류남.
...
-- 유효한 회원 등급번호가 아님에도 불구하고 잘 insert됨...                                -- MEM_GRADE에 GRADE_CODE는 10, 20, 30만 존재함.
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE--(GRADE_CODE)-- 컬럼레벨방식 : 생략하면 프라이머리키로!!
...
INSERT INTO MEM
VALUES(3, 'user03', 'pass03', '이강인', null, null, null, 40);
-- ORA-02291: integrity constraint (DDL.SYS_C007135) violated - parent key not found
...
-- > MEM_GADE 테이블에서 10번 등급 삭제!!
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = '10'; --*********************                                 -- WHERE절이 하나라도 있어야 한다. 없으면 전체 삭제.
-- ORA-02292: integrity constraint (DDL.SYS_C007135) violated - child record found
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식 테이블의 값을 
    ...
    - ON DELETE CASCADE : 부모 데이터 삭제시 해당 데이터를 쓰고 있는 자식데이터도 같이 삭제시킴
*/
...
CREATE TABLE MEMBER(
    ...
    ENROLL_DATE DATE DEFAULT SYSDATE                                             -- ENROLL_DATE : 가입일자
);
...
ALTER TABLE DEPARTMENT ADD FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;         -- LOCATION 뒤에 컬럼을 생략한 이유 : LOCATION 테이블의 PK가 LOCAL_CODE이므로 PK로 자동 참조되게끔.
