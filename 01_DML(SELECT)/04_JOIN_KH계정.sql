...
/*
    < JOIN >
    두 개 이상의 테이블에서 테이터를 조회하고자 할 때 사용되는 구문
...
SELECT *
FROM DEPARTMENT; -- DEPT_ID                                                         -- DEPT_CODE와 DEPT_ID가 연결고리
...
FROM EMPLOYEE; -- JOB_CODE                                                          -- 연결고리는 JOB_CODE
...
-- >> ANSI 구문
-- FROM 절에 기준이되는 테이블을 하나만 기술 한 후
-- JOIN 절에 같이 조회하고자 하는 테이블 기술 + 매칭시킬 컬럼에 대한 조건도 같이 기술
-- JOIN USING, JOIN ON

-- 1) 연결한 두 컬럼명이 다른 경우 (EMP : DEPT_CODE, DEP : DEPT_ID)
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB ON (JOB_CODE = JOB_CODE);                      -- 에러발생
...
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE
AND J.JOB_NAME = '대리';                                                              -- /*J.*/JOB_NAME = '대리';
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 21명만 나옴                               -- 총 23명이어야 한다.
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);                                                       -- 기준이 되는 테이블(FROM 절)의 반대되는 컬럼(WHERE 절) 뒤에 (+)를 붙인다.
...
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE,
       M.EMP_ID, M.EMP_NAME, M.DEPT_CODE
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE M ON (E.MANAGER_ID = M.EMP_ID);                                  -- 사수가 없는 사원들도 전부 출력된다.
--------------------------------------------------------------------------------
/*
    < 다중 JOIN >
...
WHERE DEPT_CODE = DEPT_ID
AND E.JOB_CODE = J.JOB_CODE;                                                        -- 컬럼명이 동일하므로 별칭 필요하다! 없으면 ambiguously 에러 발생.
...
FROM EMPLOYEE, DEPARTMENT, LOCATION                                                 -- 모든 컬럼명이 다 다르므로, 별칭 별도로 부여하지 않아도 됨.
WHERE DEPT_CODE = DEPT_ID
