...
/*
    * 서브쿼리의 구분
    ...
    - 다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러 행일 때 (여러 행 한열) => 동명이인 노옹철 2명일 때
...
SELECT ROWNUM, E.*                                                                  -- ROWNUM과 같은 컬럼이 있는 경우 * 사용 불가 => 테이블에 별칭을 부여하고 "별칭.*"
FROM (SELECT EMP_NAME, SALARY
      FROM EMPLOYEE
...
-- FROM EMPLOYEE; -- 1
-- WHERE 순위 <= 5; -- 2
-- WHERE RANK() OVER(ORDER BY SALARY DESC) <= 5;        -- SELECT절에서만 가능함!
