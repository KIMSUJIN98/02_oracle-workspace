...
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
WHERE ROUND(AVG(SALARY)) >= 3000000 -- 오류발생!! (그룹함수 가지고 조건 제시시 WHERE 절에서는 안됨!!)
GROUP BY DEPT_CODE
ORDER BY 1;
...
FROM EMPLOYEE -- 1
GROUP BY DEPT_CODE -- 2
HAVING AVG(SALARY) >= 3000000; --3                                                  -- 실행순서에 따라 추후에 SELECT 절에서 평균급여가 ROUND 처리되므로 굳이 ROUND 처리를 안해줘도 된다.
...
-- 부서별 보너스를 받는 사원이 없는 부서만을 조회 부서코드만 나오게 해보세요
SELECT DEPT_CODE -- , COUNT(BONUS)
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(BONUS) = 0;                                                            -- COUNT 형은 숫자를 반환하므로 '0'이 아닌 0! 하지만 문자형 '0'을 써도 자동형변환 되므로 문제 없음.
...
FROM EMPLOYEE
WHERE SALARY > 3000000;
-- 각 처리문의 SELECT 절에 작성되어 있는 컬럼 개수 동일해야됨!
