...
/*
    ...
    [MAXVALUE 숫자]                -- 최대값 지정 (기본값 겁나큼.. 9999999999999999...)
...
-- *Action:   select NEXTVAL from the sequence before selecting CURRVAL          -- CURRVAL을 하려면 NEXTVAL를 해라!

-- NEXTVAL를 단 한번도 수행하지 않는 이상 CURRVAL 할 수 없음!!
-- 왜? 마지막으로 성공적으로 수행된 NEXTVAL 값이기 때문에!!

-- SELECT 여러번 치지말기!!!                                                        -- SELECT 칠수록 결과값은 달라진다!
...
SELECT * FROM USER_SEQUENCES;                                                    -- LAST_VALUE 값이 300 >> 315로 바뀜! => 하지만 지정한 MAXVALUE 값이 310이므로 CURRVAL 값은 315가 아닌 310으로 유지됨.

SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 지정한 MAXVALUE 값 초과했기 때문에 오류 발생! (실패!)
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;                                              -- 여전히 310에서 끝난 상태이다.
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, SAL_LEVEL, HIRE_DATE)   -- 시퀀스를 주어줌으로써 자동으로 사원번호 부여
...
/*INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, SAL_LEVEL, HIRE_DATE)
VALUES (SEQ_EID.NEXTVAL, ?, ?, ?, ?, SYSDATE);*/                                 -- 정보를 받아 새로운 사원 추가하는 방법
