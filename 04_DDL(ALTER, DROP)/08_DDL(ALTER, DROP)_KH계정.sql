...
-- LNAME 컬럼 추가 : VARCAHR2(20) 기본값 : 한국
...
-- 디폴트값을 바꾼다고 해서 이전에 추가된 데이터가 바뀌는것은 아니다!!                        -- DEFAULT값은 '영국'으로 바뀌었지만, 데이터 값은 이전에 추가했을 당시의 DEFAULT값인 '한국'이 유지됨
...
ALTER TABLE DEPT_COPY2
    DROP COLUMN DEPT_ID
    DROP COLUMN DEPT_TITLE;                                                      -- 에러 남 / ORA-00933: SQL command not properly ended

ALTER TABLE DEPT_COPY2 DROP COLUMN CNAME;
ALTER TABLE DEPT_COPY2 DROP COLUMN LNAME;

ALTER TABLE DEPT_COPY2 DROP COLUMN LOCATION_ID;
-- 오류남!! 최소 한개의 컬럼은 존재해야됨!!                                             -- ORA-12983: cannot drop all columns in a table
--------------------------------------------------------------------------------
-- 2) 제약조건 추가 / 삭제
/*
    2_1) 제약조건 추가
...
ALTER TABLE DEPT_COPY
    ADD CONSTRAINT DCOPY_PK PRIMARY KEY(DEPT_ID)                                 -- PRIMARY KEY와 UNIQUE는 저번에 못가져왔음
    ADD CONSTRAINT DCOPY_UQ UNIQUE(DEPT_TITLE)
    MODIFY LNAME CONSTRAINT DCOPY_NN NOT NULL;                                   -- LNAME은 이번에 만든것이므로 NOT NULL 조건이 이번에 추가됨
...
-- SYS_C007190 => DCOPY_DID_NN
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007190 TO DCOPY_DID_NN;             -- '' 필요없다.

-- 3_3) 테이블명 변경 : RENAME [기존테이블명] TO 바꿀테이블명                            -- "ALTER TABLE 테이블명"을 앞에 쓰므로 기존테이블명은 생략 가능.
-- DEPT_COPY => DEPT_TEST
